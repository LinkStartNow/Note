...
> 这题就是拓扑排序的模板题，我们可以将依赖关系抽象成有向图，然后通过拓扑排序的方式判断环就能求解
...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        vector<int> e[numCourses], cnt(numCourses);
        ...
        queue<int> q;
        int count = 0;
        for (int i = 0; i < numCourses; ++i) if (!cnt[i]) q.emplace(i);
        while (q.size()) {
            int u = q.front(); q.pop();
            ++count;
            for (int v: e[u]) {
...
> 顺着刚刚的思路，我们在拓扑排序一边摘节点的同时一边记录摘下的节点，那么整个顺序也就出来了
...
> 这题直接裸的Dijkstra就完事了求从0开始到任一点的最短路径，这题还有个存活时间限制，所以假如路径长度超过了存活时间则不能到达，同时也不能通过该节点到达其他节点
...
public:
    vector<int> minimumTime(int n, vector<vector<int>>& edges, vector<int>& disappear) {
        ...
        for (const auto& v: edges) {
            // if (v[2] >= disappear[v[0]] || v[2] >= disappear[v[1]]) continue;
...
> 每个节点可能被缩短多次，也就是说可能多次入堆，而我们只需要最后一次入堆的答案，所以每次更新前先看看是不是最后一次的答案
